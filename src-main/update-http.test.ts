...
describe('buildProxyUri', () => {
  it('host+port 有效 → http://host:port', () => {
    expect(buildProxyUri({ llama_dir: '', proxy: { host: '127.0.0.1', port: 10808 } }))
      .toBe('http://127.0.0.1:10808');
  });
  it('缺 host → null', () => {
    expect(buildProxyUri({ llama_dir: '', proxy: { port: 10808 } })).toBeNull();
  });
  it('缺 port → null', () => {
    expect(buildProxyUri({ llama_dir: '', proxy: { host: '127.0.0.1' } })).toBeNull();
  });
  it('port 超范围 → null', () => {
    expect(buildProxyUri({ llama_dir: '', proxy: { host: 'h', port: 99999 } })).toBeNull();
    expect(buildProxyUri({ llama_dir: '', proxy: { host: 'h', port: 0 } })).toBeNull();
  });
  it('host 前后空白 → trim 后使用', () => {
    expect(buildProxyUri({ llama_dir: '', proxy: { host: ' h ', port: 1 } })).toBe('http://h:1');
  });
  it('无 proxy 节 → null', () => {
    expect(buildProxyUri({ llama_dir: '' })).toBeNull();
  });
});
...
describe('makeUpdateFetch', () => {
  ...
  it('有代理 → 走 undici.fetch + ProxyAgent，保留原 init', async () => {
    calls.length = 0;
    const f = makeUpdateFetch({ llama_dir: '', proxy: { host: '127.0.0.1', port: 10808 } });
...
describe('makeUpdateFetch 连接池缓存', () => {
  it('同一 uri 复用 → ProxyAgent 只创建一次', () => {
    const cfg = { llama_dir: '', proxy: { host: '127.0.0.1', port: 10808 } };
    makeUpdateFetch(cfg);
    makeUpdateFetch(cfg);
  ...
  it('uri 变化 → 关闭旧代理，新建新代理', async () => {
    makeUpdateFetch({ llama_dir: '', proxy: { host: '10.0.0.1', port: 8080 } });
    const oldAgent = createdAgents[0];
    expect(createdAgents.length).toBe(1);
    makeUpdateFetch({ llama_dir: '', proxy: { host: '10.0.0.2', port: 9090 } });
  ...
  it('关闭失败 → 不抛错阻断（close 的 Promise reject 被吞）', () => {
    ...
    makeUpdateFetch({ llama_dir: '', proxy: { host: 'a', port: 1 } });
    createdAgents[0].close = vi.fn(() => Promise.reject(new Error('close fail')));
    // 触发 swap（不同 uri），不应同步抛错
    expect(() => makeUpdateFetch({ llama_dir: '', proxy: { host: 'b', port: 2 } })).not.toThrow();
  });
});
