...
export function buildProxyUri(cfg: Pick<AppConfig, 'proxy'>): string | null {
  const host = cfg.proxy?.host?.trim();
...
export function makeUpdateFetch(cfg: Pick<AppConfig, 'proxy'>): typeof fetch {
