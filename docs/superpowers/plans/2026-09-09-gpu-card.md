...
**架构：** 主进程新增数据层 `src-main/gpu-stats.ts`：常驻 PowerShell 子进程每 2 秒采样标准性能计数器（动态层：用量 + 利用率，按 LUID 聚合），启动时一次性 DXGI 枚举（静态层：卡名 + 上限 + LUID），两层按顺序无关 canonicalLuid 合并（真机动态层 LUID 段序与静态层相反，见 §2.3 修订注）后经 `gpu-stats` 推送事件送到渲染端。渲染端新增 `src/modules/GpuModule.vue`：2 层绝对定位滑动（~200ms ease-out）、模运算绕回、底部 N 圆点纯展示。
...
- 数据源已在开发机实测验证（spec §2），本计划不重新设计数据源，只落地代码：
  - 动态层：`\\GPU Adapter Memory(*)\\Dedicated Usage` / `Shared Usage`（字节，实例名 `luid_0x{High}_0x{Low}_phys_N`——真机两段各带 0x 前缀，2026-09-09 真机实证，解析器兼容裸 hex 次备选）；`\\GPU Engine(*)\\Utilization Percentage`（实例名 `pid_X_luid_0x{High}_0x{Low}_phys_N_eng_M_engtype_3D`，同 LUID 取 max；无 3D 实例取 0）
...
```
预期：exit 0；文件为**单行合法 JSON 数组**；本机双卡 → 2 个元素，其中一个 name 为 `NVIDIA GeForce RTX 4090`（dedicatedTotal ≈ 25310527488）、另一个为 `Microsoft Basic Render Driver`（dedicatedTotal = 0）；luid 形如 `0x{8hex}_0x{8hex}`（双 0x 前缀，换机器则值不同；只校验结构：2 个元素、字段齐全）。用 node 快速校验：
```
...
```
预期：2~3 行压缩 JSON；每行含 `"ded"`、`"shr"`、`"eng"` 三个键；`ded` 的键形如 `luid_0x00000000_0x00010fbf_phys_0`（真机双 0x 前缀）；`eng` 的键形如 `pid_4260_luid_0x0000edff_0x00000000_phys_0_eng_0_engtype_3D`（pid/eng 号与 LUID 值随运行环境变化）。
...
export interface GpuDynamic {
  luid: string; // 小写 luid 串（真机双 0x 形状；字符串不透明键）
...
//   GPU Adapter Memory：luid_0x{High}_0x{Low}_phys_N（真机双 0x 前缀；兼容裸 hex 次备选）
...
// 合并规则（spec §2.3）：卡列表以动态层为准、保持动态层顺序；静态层按顺序无关 canonicalLuid join 补
// （真机动态层 0x{High}_0x{Low} 与静态层 0x{Low}_{High} 段序相反，join 键去 0x 后排序归一）卡名 + 上限；join 不上 → 回退命名 "GPU 序号"（按动态层下标 +1）+ 上限 0（UI 显示 "–"）；
...
export interface GpuStats {
  luid: string; // 小写 luid 串（真机双 0x 形状 0x00000000_0x00010fbf；字符串不透明键）
  name: string; // DXGI 卡名；join 不上回退 "GPU 序号"
  utilization: number; // 0-100
  dedicatedUsed: number; // 字节
  dedicatedTotal: number;
...
```ts
  ...
  // 跨浏览器帧等待（helper，加在文件顶部 helper 区，describe 之外）：
  // 滑动帧是 setTimeout(0) 宏任务（真实 Chromium 在定位帧绘制后才执行）。
...
运行：`npx vitest run src/modules/GpuModule.test.ts`
预期：跨帧锁定读帧点使『点 ›』『点 ‹』2 用例 FAIL（快速点击用例断言终态语义，v1 恰好通过——红=2 为预期）；任务 5 的 8 个用例仍 PASS。

- [ ] **步骤 3：替换 go() 为完整动画实现**

`src/modules/GpuModule.vue` 的 script 中：
1. import 行（最终落盘，无 nextTick）：`import { computed, onMounted, onUnmounted, ref, watch } from 'vue';`
2. `const layers = ...` 之后追加（最终落盘；无 activeLayer——槽位不变量「静止态当前卡恒在 0 层、目标恒走 1 层」下它零读点，已删）：
...
4. 整个 `function go` 替换为（最终落盘版；时序注记：happy-dom + VTU 下 trigger() 内置 nextTick，滑动帧必须是 setTimeout(0) 宏任务才能跨浏览器帧——纯 nextTick 微任务链在真实 Chromium 中同帧折叠，目标卡直接弹出（审查重要#1）；settle 在滑动帧回调内挂出，自滑动开始起算；fast path 先清 frameTimer 再清 settleTimer，防迟到帧回调破坏第二击定位；数据刷新不触发本路径，只有点击触发）：
...
> 时序依赖注记：3 个动画用例与 happy-dom/VTU 微任务顺序耦合（waitFrame=10ms 经验小等待，探针验证微任务读帧点绝不触发、10ms 必触发）；升级 Vue/VTU 或换测试环境需重跑本文件。
