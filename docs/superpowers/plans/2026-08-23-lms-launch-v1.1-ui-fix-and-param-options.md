...
**SDD 流程约定：** 每任务完成后——跑 `npx vitest run` 全绿 → git commit（提交信息格式见各任务）→ 更新 docs/superpowers/sdd/superpowers-sdd-progress.md 对应行（状态 done + 提交区间）→ commit 台账。任务顺序即依赖顺序（1→2→3…），勿并行改同一文件。
...
把 docs/superpowers/sdd/superpowers-sdd-progress.md 的 v1.1 批次表更新（任务 1 行标 done + 本次提交哈希；若表尚为旧表则整表替换）。commit：`chore: sdd ledger task 1 (v1.1)`。
...
**文件：**
- 修改（可能）：dev 中发现的零散修复；`dist-release/` 产物；`docs/superpowers/sdd/superpowers-sdd-progress.md`
...
docs/superpowers/sdd/superpowers-sdd-progress.md v1.1 批次表全部 done；commit `chore: sdd ledger v1.1 complete`。
