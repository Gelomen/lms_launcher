...
| 22 下拉/行名截断改按视觉宽度预算（CJK=2/拉丁=1：LaunchBar+TemplateModal 预算 16、TemplateModule 30——中文旧阈值零回归（8×2=16/15×2=30），英文由 ~8/~15 字符放宽到 ~15~16/~29~30 字符再 +…；新 util src/util/truncate.ts，spec 2026-08-26-truncate-by-visual-width） | done | （本提交） | ✅ TDD：util RED(模块缺失) → GREEN 7/7（charWidth/visualWidth/truncateByWidth）→ GREEN（三组件 truncateByWidth + visualWidth 判 tip）。中文旧断言全部原样通过；新增英文宽度用例（LaunchBar 2 / TemplateModal 1 / TemplateModule 2：16/30 边界不截断 + 超宽前缀+…+tooltip 全名）。vitest 82/82 PASS、build EXIT=0。目检待 GUI：英文下拉行（如 Qwen3.8-27B-Ridge）显示 ~15~16 字符带 … 不再过早省略，中文显示与之前一致 + hover tooltip 全名 |
| 23 宽度容忍 grace=2（用户复报：Qwen3.8-27B-Ridge 宽 17 只超 1 却手动砍成 Ridg…）——truncateByWidth(s,b,grace=2)：width ≤ b+g 全量渲染不手动 +…，超出才手动截断；tooltip/triggerTip 谓词同步 >b+2；.option 补 nowrap+hidden+ellipsis（面板行此前无 CSS 兜底，JS 放宽后必须防换行撑高=任务 19 根因不回退）；spec 当日第二轮追加 | done | （本提交） | ✅ TDD：util RED(3 fail)→GREEN 10/10（grace=2 默认全量 / >b+g 仍截断 / grace=0 严格回退）。组件新增 grace 用例（LaunchBar Qwen3.8-27B-Ridge 无手动 …+无 tooltip）。vitest 86/86 PASS、build EXIT=0。目检待 GUI：17 宽英文名全量显示、行内放不下时由 CSS 自动出 …（比手动估算多放几个字符）；32+ 超长名仍手动截断 + hover tooltip 全名 |
| 24 模板 id 改自动生成（新建去掉 id 输入框 + 取消 id 校验逻辑；保存时 invoke suggest_config_id 生成唯一 yaml-safe id 再 save_config；编辑模式显示只读 .id-view 文本、无输入框不可修改；主进程 config.suggestConfigId + ipcMain.suggest_config_id） | done | （本提交） | ✅ TDD：RED（TemplateModal auto-id describe 4 fail——id-input 仍渲染 / suggest 未调用 / .id-view 缺失 / suggest 错误未展示）→ GREEN（TemplateModal 去 formId/idError + 新增只读 .id-view/.id-hint + save() 新建分支 invoke suggest_config_id；config.ts suggestConfigId + main.ts suggest_config_id IPC；existingIds prop 移除）。vitest 92/92 PASS、build EXIT=0。目检待 GUI：新建弹窗无 id 输入框（「保存时自动生成」灰字）+ 编辑弹窗显示只读 id |
| 25 新建模板 id 完全静默（用户复报：无须显示「id / 保存时自动生成」——.id-hint 与共享 id label 行整体移除；编辑模式保留「id」label + 只读 .id-view） | done | （本提交） | ✅ TDD：RED（new_mode_shows_no_id_label_or_auto_hint 1 fail——.id-hint 仍渲染）→ GREEN（TemplateModal 模板层 v-if="isEdit" 包 label+.id-view，.id-hint DOM + CSS 删除）。vitest 93/93 PASS、build EXIT=0 |
| 26 修复首个模板保存报 MISSING（suggest_config_id 用 configsLoad 读现有 id → yaml 未创建时抛 MISSING；抽 existingConfigIds：文件缺失/空 → []） | done | （本提交） | ✅ TDD：RED（existing_config_ids_empty_when_file_missing 1 fail——函数不存在）→ GREEN（config.ts existingConfigIds 缺失→[] + suggest_config_id IPC 改调它）。vitest 94/94 PASS、build EXIT=0。目检待 GUI：首个模板保存成功且 yaml 自动生成 |
| 27 编辑模式 id 显示格式「id: xxx」单行（用户复报：独立 label + 值两行 → 「id: tplxxx」） | done | （本提交） | ✅ TDD：RED（edit_mode_id_shown_as_id_colon_value 1 fail——textContent 'qwen38' vs 期望 'id: qwen38'）→ GREEN（.id-view 改单行模板插值，独立 label 移除；旧断言同步合并去重）。vitest 94/94 PASS、build EXIT=0 |
| 28 模板字段 desc → name（label「描述」→「名字」+ 数据 key 全链路 desc→name：ConfigEntry.name、configsLoad 归一 legacy desc（任意保存后固化）、saveConfigEntry 存 name key；LaunchBar/TemplateModule/TemplateModal 读取改 c?.name） | done | （本提交） | ✅ TDD：config RED（name key + legacy 归一 2 fail）→ GREEN（normalizeEntry）；渲染端 RED（LaunchBar 5 fail——mock name 组件读 desc）→ GREEN。vitest 96/96 PASS、build EXIT=0。目检待 GUI：弹窗 label「名字」+ 存量 yaml 保存后 desc 变 name |
