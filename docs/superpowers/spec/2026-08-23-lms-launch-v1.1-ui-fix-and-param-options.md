...
| 2 | 模板模块 | 交互 | 无模板时红色错误 → 深色普通文案「暂无模板配置」 |
...
- get_configs MISSING（isMissing）：**不显示红色**，显示一行深色（label/正文字色）普通文案 **「暂无模板配置」**；不再展示原始错误串。
...
- MISSING / configs 为空：**完全移除**该模块的提示文字（不显示任何红字或普通提示行）；下拉菜单占位项改为 **「暂无模板配置」**。
