...
- 首帧数据到达前：卡片主体留空（仅「GPU 信息」标题），无标题行、无四格、无圆点
  （2026-09-10 修订：原 "…" 占位态尚不知卡数，无法预知 .gpu-body--nav 让位是否生效，
  多卡机器数据到达后内容区左右收窄 32px 造成标签/数值跳位；改留空后布局唯一。
...
- src/modules/GpuModule.test.ts：
  - 首帧留空（2026-09-10 修订，原 "…" 占位；多卡让位跳位根因）
  - 数据到达后四格数值 + 合计行正确
  - 单卡：无 ‹ › 按钮，单点实心
