...
展示顺序（2026-09-10 用户调整，2×2 网格行优先）：
...
│  ‹     当前卡名（层顶行，随卡滑动）       ›  │
│      专用 GPU 内存        共享 GPU 内存    │
│      22.7/24.0 GB         0.8/47.7 GB    │
│        GPU 内存             GPU 利用率    │
│       23.5/71.7 GB            28 %       │
│               ● ○ ○                     │
└──────────────────────────────────────────┘

- 卡片无 h2 标题（原「GPU 信息」已删除，用户 2026-09-10）；标题行 = **层顶行**（原 h2 所在行），
  当前卡名始终显示（单卡也显示，用户指定）；行**在滑动层内**（每层一份，
  切卡时与四格同步滑动，动画结束停在当前卡名）——用户 2026-09-11 指定
  （见 docs/superpowers/changes/2026-09-11-gpu-title-slide.md）；
  卡名恒左右居中；四格标签/数值恒左右居中对齐（用户 2026-09-10）；
  四格顺序 专用/共享/GPU 内存/GPU 利用率（2×2 行优先，用户 2026-09-10 调整）
- 多卡时：‹ 按钮贴卡片**左边缘**、› 按钮贴**右边缘**（左右各占一边，纵向居中，用户指定）；
  按钮**恒渲染**（单卡/首帧禁用不可点击，用户 2026-09-10 批注），内容区恒预留 32px 让位
- 让位恒生效（.gpu-body padding: 0 32px），避免与内容重叠；首帧占位与数据态位置结构因此一致
- 四格数值区：2×2 网格，标签小字 + 数值大字，复用现有 .card 内部既有排版类
- 底部指示点：N 张卡 = N 个圆点；当前卡 = 实心灰点，其余 = 灰色空心（描边）点。纯展示，不可点击（切换只走 ‹ ›）
- 首帧数据到达前：占位与数据态位置结构恒一致（2026-09-10 修订）——标题行 "–"、
  利用率 "–"、专用/GPU 内存/共享 三格 "– / –"、‹ › 恒渲染但 disabled、无圆点（卡数未知）。
  内容区恒预留 32px 按钮让位（.gpu-body 基类，单卡/多卡/首帧一律同宽），
  单卡时 ‹ › 禁用不可点击——全部状态结构唯一，零跳位。
  见 docs/superpowers/changes/2026-09-10-gpu-first-frame-placeholder.md
...
- 动画：内容区两张绝对定位层（当前层 currentLayer / 目标层 nextLayer）；
  **层内容 = 标题行 + 四格网格**（标题行在层内，与四格同 transform 同步滑动——
  2026-09-11 修订，原为舞台级标题行点击立即切换）。
  - 点 ›：当前层 translateX(0 → 屏外左侧停靠位)，目标层 translateX(屏外右侧停靠位 → 0)
  - 点 ‹：方向相反（0 → 屏外右侧停靠位，屏外左侧停靠位 → 0）
  - 屏外停靠位 = ±(100% + 相邻卡片间距)；间距（用户 2026-09-11 指定）= 2 × 内容边缘到卡片边缘的距离
    （多卡 32px 按钮让位 + 16px 卡片内边距 = 48px，即 96px）——相邻卡片始终隔着固定距离滑动，
    settle 隐藏离场层时无"紧贴后突然消失"的跳变
  - 时长 400ms（原 ~200ms 的 0.5 倍速，用户 2026-09-11 调整），ease-out；切换结束（transitionend 或 setTimeout）后隐藏闲置层、重置 transform
...
- src/modules/GpuModule.test.ts：
  - 首帧占位（2026-09-10 修订：标题 "–"、利用率 "–"、内存格 "– / –"、‹ › 渲染但 disabled、无圆点）
  - 数据到达后四格数值 + 合计行正确
  - 单卡：‹ › 渲染但禁用，单点实心
